...
const fixtureUrl = new URL("../fixtures/v0.1/explicit-definition.json", import.meta.url);
const goldenUrl = new URL("../fixtures/v0.1/explicit-definition.golden.json", import.meta.url);
...
if (nativeJson !== wasmJson) throw new Error("native/WASM semantic result mismatch");
const golden = JSON.parse(await readFile(goldenUrl, "utf8"));
if (nativeJson !== JSON.stringify(golden.results)) {
  throw new Error("semantic result differs from the v0.1 golden fixture");
}
console.log(`parity OK: ${fixture.queries.length} v0.1 queries`);
