...
// Public acceptance examples, not a sealed holdout or a field-coverage score.
const cases = [
  {
...
function verify(item, source, results, label) {
  ...
  if (item.unproved) {
    assert.ok(["partial", "unsupported", "conventional"].includes(semantic.view.authoringContext.disposition), `${label}: unexpected formula authority`);
    assert.ok(semantic.view.authoringContext.interpretations.hypotheses.every((entry) => entry.label !== "red" && entry.label !== "blue"), `${label}: unrelated alternatives`);
  }
  if (item.definition === true) {
